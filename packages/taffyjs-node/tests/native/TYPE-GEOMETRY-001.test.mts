...
contractTest("TYPE-GEOMETRY-001/style-partial", () => {
...
contractTest("TYPE-GEOMETRY-001/components", () => {
  const style = storedStyle({
    overflow: { x: 1, y: 2 },
...
contractTest("TYPE-GEOMETRY-001/style-shape-errors", () => {
  for (const style of [
    { overflow: [1, 2] },
...
contractTest("TYPE-GEOMETRY-001/scalar-scope", () => {
  for (const field of [
    "inset",
    "size",
    "minSize",
...
contractTest("TYPE-GEOMETRY-001/detached-reuse", () => {
  const owner = createOwner();
