...
- `packages/` contains independent JavaScript package boundaries. `packages/taffyjs-node` owns the generated ESM loader and types, rare package-local unit tests, and npm metadata for `@taffyjs/node`.
...
Platform-specific native packages use the `@taffyjs/binding-<platform>` naming scheme while `@taffyjs/node` remains the public root loader and entry point.

## Development

Install dependencies with `vp install`, then run `vp run ready` to execute the uncached Vite+ task graph for formatting, linting, Rust checks, native build, and JavaScript tests. Authored JavaScript and TypeScript libraries use `vp pack`; the napi-rs-generated native loader is already distributable and is not repacked.
