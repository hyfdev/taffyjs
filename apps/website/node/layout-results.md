...
| `contentSize`   | Width and height of the node's laid-out content.      |
...
`DetailedLayoutInfoKind.None` has no payload. As with ordinary layouts, this getter returns stored data rather than computing or proving freshness. Taffy 0.13 can retain earlier Grid detail after a later non-Grid computation, so use it in the same update step as the Grid computation whose details you intend to inspect.
