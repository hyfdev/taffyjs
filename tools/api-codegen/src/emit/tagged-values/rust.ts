...
export function emitTaggedValuesRust(model: TaggedValuesModel): OutputFile {
  const families = model.families.filter((family) => family.emitRustParser);
  const numericCodes = families.map((family) => family.numericFamily.rustName).toSorted();
  const hasNumberShorthand = families.some((family) => family.numberShorthand !== undefined);
  const numericImports =
    numericCodes.length === 1
      ? [`use crate::numeric::${numericCodes[0]};`]
      : numericCodes.length > 1
  ...
  for (const family of families) emitFamily(lines, family);
