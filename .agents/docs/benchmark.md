...
Both commands use the same TypeScript coordinator and benchmark implementations. Package metadata does not duplicate their orchestration as compound scripts. Timing runs serially because concurrent benchmark processes would interfere with one another. Official measurements use actual release artifacts, repeated independent samples, and recorded repository, package, host, and Node.js revisions. A publication may run on a different machine than the one before it, so every dataset carries its own environment. The exact sampling and summary statistic remain open until the runner design is selected.
