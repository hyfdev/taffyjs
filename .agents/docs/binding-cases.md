...
The public boundary treats Style as an ordinary data object. The generated encoder checks the supplied property names once, reads every known property once, and keeps its byte storage owned through the synchronous native call. Methods consuming NodeIds resolve them before encoding so getter or Proxy re-entry cannot change stale or foreign ID precedence; nested Style operations receive separate temporary storage.
...
| Unknown fields           | Reject unknown enumerable string field names on the outer `StyleInput` and on partial geometry records with a controlled argument error.                                                                                                             | A misspelled or unsupported layout field is ordinary user error whose silent defaulting changes layout. The public JavaScript wrapper and its platform package are version-coupled, so silently accepting a field for cross-version forwarding is not needed. The name check costs one key walk per call: the `styled-node-construction-500-leaves` benchmark puts it at 3.6% of a Native transaction and inside the run-to-run noise on WASI. Symbols and unrelated nonenumerable metadata need no Style meaning. |
...
The container rules are demonstrated by ordinary integration cases: `{}` and explicit `undefined` produce Taffy defaults; `setStyle(node, {})` resets a previously nondefault Style instead of merging; a partial fixed-shape nested record fills each omitted component from the corresponding enclosing Style default rather than stored state; an unknown top-level field, an unknown partial-record component, and `null` for a required field each throw without changing the stored Style; `null` succeeds only for a publicly nullable field; a complete output uses `null` rather than an omitted or undefined field for Rust `None`; and declarations accept explicit `undefined` under `exactOptionalPropertyTypes` and retain the required property-level JSDoc. Focused tests cover getter, Proxy, and recursive operations across all five Style-taking methods.
...
Each tagged branch declares only the fields that its selected variant needs. It does not add `value?: never`, `index?: never`, or similar exclusions for other variants. Conversion reads the selected discriminator and its required payload; unrelated extra structural properties are caller responsibility and are absent from canonical output. This rule is separate from the strict field names required on the outer `StyleInput` and on partial geometry records.
