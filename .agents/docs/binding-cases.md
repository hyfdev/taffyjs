...
Snapshot materialization is deliberately outside this case series. The separate [architecture decision](architecture.md#complete-owned-snapshots) retains complete eager snapshots, while [output optimization research](output-optimization-research.md) preserves lazy and selective experiments without turning them into an initial API or another alignment case.
...
The `calc` feature is enabled by Taffy's default features, but its public length types represent calc values through opaque pointers. The high-level `TaffyTree` implementation resolves every calc pointer to `0.0`; it does not expose an application resolver. Raw calc pointers therefore cannot define a JavaScript Style value, so the selected public high-level Style vocabulary excludes calc. Whether Cargo also disables Taffy's calc feature remains an implementation choice.
...
The baseline treats Style as an ordinary data object. It performs one NodeId validation and does not add another validation or a special defensive Style copy solely for getter or Proxy side effects. Native input conversion still produces one complete Rust Style before this operation mutates the tree, but it need not run before the JavaScript registry lookup solely to defend against re-entry during property access.
...
| Unknown fields           | Reject unknown enumerable string field names with a controlled argument error.                                                                                                                                                                       | A misspelled or unsupported layout field is ordinary user error whose silent defaulting changes layout. The public JavaScript wrapper and its platform package are version-coupled, so silently accepting a field for cross-version forwarding is not needed. Symbols and unrelated nonenumerable metadata need no Style meaning. |
...
| Conversion and mutation  | Validate ordinary wrapper-owned record rules, perform the wrapper's one current-node lookup, then call native synchronously; native conversion must produce a complete Rust Style before replacing tree state.                                       | The operation reaches native mutation only with a complete Rust value, without adding a repeated lookup or accessor/Proxy-specific copy.                                                                                                                                                                                          |
...
The repeated type machinery belongs in one shared abstraction rather than in handwritten unions for every family. The public `EnumValue` helper and illustrative internal factory below have the required inference and selected naming behavior under the repository's pinned TypeScript version:

```ts
export type EnumValue<Family extends Readonly<Record<string, number>>> = Family[keyof Family];

function defineNumericEnum<const Members extends Readonly<Record<string, number>>>(
  members: Members,
): Readonly<Members> {
  return Object.freeze(members);
}

export const Display = defineNumericEnum({ Flex: 0, None: 1 });
export type Display = EnumValue<typeof Display>;
```

The helper removes repeated TypeScript assertions, but it does not by itself prevent JavaScript constants, declarations, native validation, and Taffy conversion matches from drifting. Implementation must evaluate one authoritative enum definition that can derive those artifacts, such as a Rust macro integrated with declaration generation or a neutral schema consumed by both sides. The selected public behavior does not depend on which generation route wins.
...
`Display` is the literal union `0 | 1` in this example. A contextually typed `display: 1` or a `const` literal therefore works directly, while `display: 99` and a widened general `number` fail ordinary declaration checks. The accepted raw form is intentionally not the recommended authoring style: callers should write `Display.None`, and public examples must not use `1` except when explaining or testing this boundary. The types are structural, so a member of another family also fits when it has the same literal code. Runtime provenance is not part of the contract, so any valid raw code behaves exactly like the recommended exported member, while an invalid code produces a controlled argument error before mutation. Complete output returns the same numeric member value, so comparison with the exported constant works and logs and JSON remain numeric.

The acceptance matrix for every fieldless closed family is: its exported member and each corresponding raw literal type-check and round-trip; public usage examples use the exported member and any boundary example labels the raw literal as not recommended; an unknown literal and a widened general `number` fail declaration tests; a same-code member from another family is structurally accepted; strings, `NaN`, infinities, fractions, and unknown integers fail at runtime without mutation; and changing Taffy's internal declaration order does not change the binding codes. Tagged-record tests must additionally prove that comparison with either the recommended constant or the equivalent raw literal narrows the complete union through ordinary TypeScript control flow.
...
Grid introduces no new boundary category. Its fieldless `GridAutoFlow` family exposes `Row`, `Column`, `RowDense`, and `ColumnDense` numeric constants whose type is their literal union. Its data-carrying variants use ordinary numeric-tagged records with the same discriminator rule, its fixed geometry uses the selected partial `Line` input, its floats use the selected `f32` pass-through, its Rust integers use exact representability checks, and its collections are copied ordinary JavaScript arrays. The declarations below show the selected mutable input vocabulary; every unsuffixed output type is the complete recursive readonly mirror using stored values.
...
The Style fields `gridTemplateRows` and `gridTemplateColumns` use arrays of `GridTemplateComponentInput`; `gridAutoRows` and `gridAutoColumns` use arrays of complete `TrackSizingFunctionInput`; `gridTemplateRowNames` and `gridTemplateColumnNames` use nested string arrays; and nullable `gridTemplateAreas` uses the normalized record shown above or `null`. Input arrays are ordinary mutable arrays. Output arrays and every nested record are recursively readonly, contain the actual stored strings and values, and use the corresponding unsuffixed types. Conversion copies every nested string, record, and array and finishes the complete Style before mutation.
...
Each tagged branch declares only the fields that its selected variant needs. It does not add `value?: never`, `index?: never`, or similar exclusions for other variants. Conversion reads the selected discriminator and its required payload; unrelated extra structural properties are caller responsibility and are absent from canonical output. This rule is separate from the explicit unknown-field rejection on the outer `StyleInput` and partial geometry records.

Acceptance cases cover every discriminator and convenience, complete pair normalization, user-facing percentage conversion, canonical output with no helper history, exact `i16` and `u16` boundaries, rejection of fractional and out-of-range integer payloads, zero-value pass-through, arbitrary JavaScript strings, nested mutable input and recursive readonly output, safely forwardable empty and nested collections, nullable template areas, ignored unrelated tagged-branch properties, full conversion before replacement, the specific empty-repeat-line-names underflow regression in both axes, and the remaining pinned Taffy panic audit. CSS-invalid structure is not by itself a rejection reason, but any concrete collection relationship found to reach a panic must be rejected or converted safely under the binding-wide safety rule.
...
These questions may matter elsewhere in the API, but resolving them would not add reference value to Case 2 and does not keep this example open:

- The authoritative shared-definition mechanism for closed enums.
- The concrete stable numeric codes assigned to the selected numeric families.
- Whether another callback contract should request less data; output optimization is deferred and does not change the complete direct measure callback.
...
The direct context and invalidation surface uses `newLeafWithContext(style, context)`, `getNodeContext(node)`, `setNodeContext(node, context)`, `markDirty(node)`, and `isDirty(node)`. The context parameter and result use `TContext | undefined` under the already vouched absence rule. `getNodeContext` returns the exact JavaScript value stored by the caller rather than a copy, readonly view, or binding-produced snapshot; JavaScript already provides ordinary mutable access, so Rust's `get_node_context_mut` and `get_disjoint_node_context_mut` do not become separate public operations.

`setNodeContext` keeps the JavaScript registry and Taffy's unit-valued presence marker consistent, returns `void`, and always marks the node dirty even if the supplied JavaScript value is referentially or primitively equal to the previous value. The binding must not compare context values to suppress Taffy's invalidation behavior.

In-place mutation of a stored context object and changes to data captured by the per-compute measure callback are invisible to the binding. The caller must invoke `markDirty(node)` for every affected node before relying on another computation. Supplying a different callback object does not automatically dirty nodes: callback identity is not part of Taffy's cache key, and treating a newly allocated function as invalidation would disable normal cache reuse. `markDirty` accepts any valid node and directly preserves Taffy's behavior of clearing that node's cache and propagating invalidation through its ancestors; it does not copy Yoga's measured-leaf-only restriction.
...
After Taffy returns, the binding clears the caches of every node in the requested root's subtree, with Taffy's ordinary ancestor propagation, so a later compute cannot reuse results produced through the internal fallback. `computeLayoutWithMeasure` then throws synchronously. A value thrown by the callback is propagated without a new taffyjs wrapper; a malformed return produces an ordinary `TypeError` without a dedicated stable binding error code. The exact diagnostic text is not a compatibility promise.
