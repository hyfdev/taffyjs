...
**Limits:** This reopens only the compact private constraints ABI / numeric-slots exclusion in [On-demand Style in measure callbacks](#on-demand-style-in-measure-callbacks). Public `MeasureArgs` type, fields, values, and `getStyle` behavior stay unchanged, and its values stay ordinary mutable objects created per request. It does not approve a public buffer or numeric-slot API, per-node native callback tables, an upstream algorithm change, a JavaScript Style mirror, or unsafe callback-scope handles.
...
**Source:** Yunfei (`@hyfdev`), 2026-08-22 and 2026-08-23; in-session answer “批准改动 A（推荐）”, then “公共 api 的表示，和内部传输不用耦合在一起吧” with the instruction to apply it to this change.
