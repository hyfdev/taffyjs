...
**Source:** Yunfei (`@hyfdev`), 2026-08-14; explicitly confirmed the proposed selector spelling and TypeScript inference requirements and asked that they and all preceding selective-query decisions be vouched.
...
The canonical public-shape representation, concrete generated declaration structure, private dispatch shape, exact malformed-call error classes, supported index range, batch size limits, and performance acceptance thresholds remain open. The public method names, selector spelling, overload behavior, and required TypeScript inference are decided. The settled boundary and the questions intentionally deferred to implementation work are recorded in [Selective query design notes](query-api-design.md).
