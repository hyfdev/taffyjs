...
`@taffyjs/wasm` uses the same Rust crate through napi-rs's threadless `wasm32-wasip1` target. The pinned CLI generates eager Node and deferred ESM loaders plus the release Wasm binary as build inputs. The package build encodes that binary into one generated ESM payload, publishes the deferred loader unchanged, and mechanically derives a synchronous CommonJS factory from the eager loader with exact-match drift assertions. The no-TLA Node ESM bridge imports the payload and immediately calls that factory; the browser adapter imports the same payload directly. Raw binaries and unadapted eager loaders are removed from the final package, and the generated target package is not published. napi-rs 3.8.2 still requires the raw declaration output passed to `--dts` to use a `.d.cts` filename.
...
After napi-rs builds, `tools/inline-wasm-package.mjs` validates and base64-encodes the release binary, generates the small host adapters, and transforms the freshly generated eager Node loader only at asserted input and WASI-construction sites. After Vite+ builds both public entries, `tools/finalize-wasm-package.mjs` removes files from `dist` that are not in the package manifest's explicit publish list, including raw release or stale debug binaries and unadapted eager loaders. This ensures the npm package contains one payload, the synchronous derived Node lifecycle, and the unmodified deferred browser loader.
