...
For `packages/taffyjs-yoga`, `vp pack` builds the root and `/load` entries plus shared chunks and declarations over the public `@taffyjs/node` dependency. Its build cleans the complete output directory first, and CI freshness checks include untracked chunks before comparing the committed artifacts.
...
The aggregate `check` task completes its formatting scan before invoking the build-backed test graph. This prevents the formatter from observing napi-rs transaction directories or generated package files while a concurrent native build is replacing them; invoking `check:test` directly remains independent of formatting.
