...
The Yoga Wasm build, package, Node.js, and browser checks live in the Linux-only `check:wasm` graph because they require the WASIP Rust target and Chromium. Both publication workflows run that graph; the Yoga workflow then runs the alternate-runtime smokes against the package it produced. Ordinary native CI continues to own `@taffyjs/yoga` on Linux and Windows without building Wasm artifacts.
