...
The landing page speaks for TaffyJS as a family and presents the intended complete product rather than serving as a release-status page. It may describe native, WebAssembly, and Yoga-compatible packages without labeling unfinished packages as planned; Guide and package documentation own concrete setup, runtime support, and public behavior.

The home page is only a landing page and uses VitePress's default theme. It contains a concise performance-led introduction, feature summaries of native performance, WebAssembly support and Yoga compatibility, and direct routes to real package documentation and source. Those three features are the primary product messages: the Node package runs native Rust without mirroring layout state in JavaScript, the Wasm package carries the same JavaScript API into its supported environments, and the Yoga packages preserve Yoga's API shape for migration. Tutorials, complete examples, runtime details and API reference belong to Guide or package documentation rather than the home page. The home page does not use invented testimonials, unsupported benchmark numbers or generic feature text to fill space.
...
- `@taffyjs/yoga` is the Yoga 3.2.1 compatibility facade built on `@taffyjs/node`. It has a distinct consumer model and therefore owns its own compatibility, migration and API documentation.
- `@taffyjs/yoga-wasm` builds that same compatibility facade over `@taffyjs/wasm`. It shares Yoga-facing source, declarations and compatibility classifications with `@taffyjs/yoga` while owning only its WebAssembly runtime and loading differences.

No planned package receives empty reference pages or documentation navigation that makes it look installable. The landing page may present the intended package family without progress labels, but a package receives a visible documentation section only when there is enough implementation and installation evidence to make that section useful. All four implemented packages now meet that threshold.
...
The `@taffyjs/node` overview owns supported Node.js and platform combinations, native import rules, and package scope. The rest of that section is the canonical human-written reference for the direct `TaffyTree`, `Style`, layout result, value helper, and error APIs shared by Node and Wasm. The `@taffyjs/wasm` section owns availability, setup and observable runtime differences and links back to that shared reference instead of duplicating it. The `@taffyjs/yoga` section owns its Yoga-compatible surface, migration guidance, compatibility scope, and behavioral differences from the direct API. The `@taffyjs/yoga-wasm` section owns only the runtime and loading differences introduced by putting that same facade over Wasm; it links to the shared Yoga compatibility material instead of copying it.
