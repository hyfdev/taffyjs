...
Until `@taffyjs/wasm` exists, Getting Started uses the runnable Node example and a static process diagram rather than fake browser controls. The page reserves that exact example position for a later interactive control which varies available space while keeping the tree and styles fixed; the interactive form must reveal the same relationship and must not replace or fork the canonical Node code.
