...
A normal named JavaScript record is read or written through a binding-local napi-rs type, using `#[napi(object)]` and an explicit input-only or output-only direction. Positional or transparent values may use `#[napi(array)]` or `#[napi(transparent)]` when those shapes are already the selected public representation. Taffy types come from another crate and do not receive napi-rs attributes directly: the binding converts between the local napi-rs type and the Taffy type. Handwritten code is limited to rules that the generated conversion cannot express accurately, such as numeric discriminators, exact integer ranges, selected unknown-field rejection, `null` distinctions, stable JavaScript error classes, callback failure, and panic containment. A native method may still accept `Unknown` and invoke the generated local conversion after acquiring checked tree access when conversion timing affects the selected same-tree re-entry behavior; that does not justify handwritten property-by-property parsing.
...
Keep public inputs as readable records and tagged unions and keep user-observable outputs as readable owned values. Compact input transport is a separate optimization, not part of the direct binding implementation: benchmark JavaScript-side conversion before replacing generated Node-API record conversion, pass small fixed inputs as positional primitive values only when measurements justify it, and consider a reusable compact `Buffer` only for large or deeply nested inputs such as `StyleInput`. Choose the cutoff from retained end-to-end Taffy workloads, include JavaScript validation and buffer-writing cost, preserve the existing public errors and mutation behavior, and do not expose or retain the private transport format. Leave native-to-JavaScript output transport unchanged until it is measured as a separate problem.
