...
Native update conversion produces an owned patch and component-presence masks before borrowing the tree. Under Taffy 0.13, `TaffyTree::style` exposes only `&Style` and `set_style` accepts a complete replacement; there is no public mutable, take, swap, or closure-based Style operation. The binding therefore compares the converted patch against the stored Style first. An empty or unchanged update returns without cloning or dirtying. A changed update starts from the owned patch value, clones only omitted fields and geometry components from the current Style, validates that complete candidate, and calls `set_style` once. Supplied collections move into the candidate without cloning the collection they replace; a scalar-only change still clones every preserved Grid vector. Safe reuse of the current Style's allocations is not available through the pinned Taffy API. A future upstream closure-style mutation API that owns dirty propagation could remove this remaining copy without changing the JavaScript contract.
