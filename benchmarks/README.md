...
The initial suite contains two scales of the same nested UI family, one broad wrapping collection, and initial-layout and persistent-resize transactions for a measured coding-agent chat model. Each scenario directory owns its workload and documents its exact timed boundary. The result is scenario-first: `yoga-layout` is the explicit `1.00×` reference within each table, and unrelated scenarios are never combined into an overall score.
