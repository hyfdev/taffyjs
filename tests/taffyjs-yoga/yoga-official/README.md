...
The snapshot contains 570 cases. Yoga 3.2.1 marks 17 of them skipped. `expected-failures.ts` classifies the 84 remaining cases that intentionally cannot pass under the published TaffyJS compatibility boundary: 77 exercise Unsupported capabilities and 7 exercise Known Differences. Vitest treats those cases as required failures, so an unexpected failure or an expected failure that starts passing both stop the suite and require a compatibility review. The maintained TaffyJS tests remain responsible for asserting the exact replacement behavior of Different and Unsupported cases.
