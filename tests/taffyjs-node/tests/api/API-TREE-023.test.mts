...
contractTest("API-TREE-023/complete-replace", () => {
...
contractTest("API-TREE-023/undefined-null", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-023/unknown-calc", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-023/conversion-families", () => {
  const tree = new (TaffyTree())();
  const node = tree.newLeaf({});
...
contractTest("API-TREE-023/dirty", () => {
  const tree = new (TaffyTree())();
  const node = tree.newLeaf({});
...
contractTest("API-TREE-023/failure-atomic", () => {
  const tree = new (TaffyTree())();
  const node = tree.newLeaf({ flexGrow: 2 });
  tree.computeLayout({ root: node, availableSpace: maxContentSpace() });
...
contractTest("API-TREE-023/invalid-id", () => {
