...
contractTest("API-TREE-013/replace-order", () => {
...
contractTest("API-TREE-013/reparent", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-013/detach-omitted", () => {
  const tree = new (TaffyTree())();
  const [first, second, third] = [tree.newLeaf({}), tree.newLeaf({}), tree.newLeaf({})];
...
contractTest("API-TREE-013/dirty", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-013/topology-reject", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-013/invalid-middle", () => {
...
contractTest("API-TREE-013/failure-atomic", () => {
  const tree = new (TaffyTree())();
  const [first, second, third] = [tree.newLeaf({}), tree.newLeaf({}), tree.newLeaf({})];
  const parent = tree.newWithChildren({}, [first, second]);
