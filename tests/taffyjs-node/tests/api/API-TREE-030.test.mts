...
contractTest("API-TREE-030/callback-args", () => {
...
contractTest("API-TREE-030/result-f32", () => {
...
contractTest("API-TREE-030/cache-calls", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-030/same-tree-busy", () => {
  const fixture = fileURLToPath(
    new URL("./fixtures/api-tree-030-same-tree-busy.mjs", import.meta.url),
  );
...
contractTest("API-TREE-030/js-only-reentry", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-030/different-tree", () => {
...
contractTest("API-TREE-030/throw-identity", () => {
...
contractTest("API-TREE-030/malformed-result", () => {
  for (const result of [
    null,
    [],
    { width: 1 },
...
contractTest("API-TREE-030/zero-drain", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-030/layout-nontransactional", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-030/context-identity", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-030/recovery", () => {
  const tree = new (TaffyTree())();
  const node = tree.newLeafWithContext({}, true);
