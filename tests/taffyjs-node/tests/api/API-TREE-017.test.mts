...
contractTest("API-TREE-017/replace", () => {
...
contractTest("API-TREE-017/returned-id", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-017/dirty", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-017/same-noop", () => {
  const tree = new (TaffyTree())();
  const child = tree.newLeaf({});
  const parent = tree.newWithChildren({}, [child]);
...
contractTest("API-TREE-017/reject", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-017/id-roles", () => {
...
contractTest("API-TREE-017/failure-atomic", () => {
  const tree = new (TaffyTree())();
