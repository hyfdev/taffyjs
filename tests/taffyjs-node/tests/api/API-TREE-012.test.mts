...
function TaffyTree(): TreeConstructor {
  ...
  assert.equal(
    typeof Reflect.get(value.prototype, "insertChildAtIndex"),
    "function",
    "insertChildAtIndex is public",
  );
...
contractTest("API-TREE-012/positions", () => {
...
contractTest("API-TREE-012/end-bound", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-012/index-errors", () => {
  const tree = new (TaffyTree())();
  const parent = tree.newLeaf({});
...
contractTest("API-TREE-012/id-roles", () => {
  ...
  assert.equal(
    captureError(() => tree.insertChildAtIndex(parent, 0, 0n)).code,
...
contractTest("API-TREE-012/failure-atomic", () => {
  const tree = new (TaffyTree())();
  const child = tree.newLeaf({});
