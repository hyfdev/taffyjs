...
contractTest("API-TREE-029/lifecycle", () => {
  const tree = new (TaffyTree())();
  const child = tree.newLeaf({});
...
contractTest("API-TREE-029/style", () => {
...
contractTest("API-TREE-029/context", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-029/topology", () => {
  {
    const { tree, child, parent, root } = settledTree();
...
contractTest("API-TREE-029/explicit", () => {
  const { tree, child, parent, root } = settledTree();
...
contractTest("API-TREE-029/child-nuance", () => {
  const tree = new (TaffyTree())();
  const child = tree.newLeaf({
...
contractTest("API-TREE-029/invalid-id", () => {
