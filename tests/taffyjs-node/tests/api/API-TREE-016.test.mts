...
contractTest("API-TREE-016/ranges", () => {
...
contractTest("API-TREE-016/detached-live", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-016/range-errors", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-016/extra-properties", () => {
  const tree = new (TaffyTree())();
...
contractTest("API-TREE-016/failure-atomic", () => {
  const tree = new (TaffyTree())();
  const [first, second, third] = [tree.newLeaf({}), tree.newLeaf({}), tree.newLeaf({})];
  const parent = tree.newWithChildren({}, [first, second, third]);
